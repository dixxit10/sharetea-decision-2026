...
if check_password():
    ...
    with def_col1:
        st.markdown("<div class='definition-box'><b>SFS 戰略總分</b><br>量化地段潛力的核心指標。整合消費力、族群權重、空間壓力與 AI 視覺診斷。</div>", unsafe_allow_html=True)
    ...
    with def_col2:
        ...
        st.markdown("<div class='definition-box'><b>月均基礎消費力</b><br>連動 CENSUS API。代表普查區之月收入中位數，決定產品溢價空間。</div>", unsafe_allow_html=True)
    with def_col3:
    ...
    def get_vision_analysis(image_bytes, sfs_context, api_key):
        try:
            ...
            數據現狀：{sfs_context}
            核心任務：識別地圖中是否有加油站(76 Gas)、汽修廠(Pep Boys/AutoZone) 等質感斷層。
            1.【環境基因診斷】：鄰里是否存在明顯美學雜訊？
            2.【執行方向】：針對 25-34 歲社交主力，該如何透過家具佈局或視覺屏障建立 Clarity？
            """
    ...
    if st.sidebar.button("Execute Strategic Analysis"):
        if not coord_input: st.error("請提供座標資料。")
            ...
            try:
                ...
                with st.spinner("正在聯動數據與執行視覺噪音稽核..."):
                    result = get_census_data(lat, lng, CENSUS_KEY)
                    if result == "COORD_ERROR":
                        st.error("❌ 座標錯誤：在美國區域，經度應為負數。")
                        st.stop()
                    elif result == "GEO_ERROR" or result is None:
                        st.error("❌ 無法獲取普查數據。請確認座標是否在美國境內。")
                        st.stop()
                    ...
                    with d2:
                        ...
                        if final_sfs > 10000:
                            behavior = "目的型社交消費"
                            advice = "💎 戰略建議：強化視覺降噪 (Visual Noise Cancellation)、配置 A+ 級視覺屏障模組，以抵銷外部機能雜訊。"
                        else:
                            behavior = "便利驅動消費"
                            advice = "🛵 戰略建議：優化點單與取貨動線之清晰度、強化社區積點聯動。"
                        
                        st.write(f"當前模式：**{behavior}**")
                        st.info(f"距離上一級門檻尚有 {gap_pct:.1%}。人均空間為 {area_per_seat:.1f} sq. ft.。")
                        st.warning(advice)

                    st.divider()
                    st.subheader("🤖 Gemini 視覺診斷與轉型建議")
                    with st.spinner("Gemini AI 正在透過地圖掃描鄰里基因..."):
    ...
    st.caption("Produced by Marketing Designer. v8.7.6 | Reducing Noise. Increasing Clarity.")
