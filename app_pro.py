...
# --- 1. UI 與 品牌視覺配置 ---
st.set_page_config(page_title="Sharetea Express 決策引擎 v4.0", layout="wide")
...
# --- 2. 戰略名詞定義 (展開設計) ---
st.title("🧋 Sharetea Express 決策引擎 v4.0")
    ...
    if zoning_factor == 0: st.error("🛑 法律排除：純住宅區禁止經營。")
        ...
        try:
            ...
            with st.spinner("正在除噪並計算 27 組演算數據..."):
                ...
                social_r = age_dict["25-34 歲 (社交主力)"]

            # --- SFS 演算 ---
            target_index = (eth_dict["華裔/台灣裔"] * 2.0) + (eth_dict["墨西哥裔/西裔"] * 1.5) + (social_r * 2.5)
            ...
            if final_sfs >= T_HOT: level, color, advice = "熱區指標 (Hotspot Index)", "gold", "【熱區指標】建議：維持旗艦美學識別與高坪效。戰略鞏固：強化品牌標竿地位。"
            elif final_sfs >= T_STD: 
                level, color = "社區標準 (Community Standard)", "orange"
                advice = "【社區標準】建議：提升品質與便利性。轉型路徑：建議強化『美學識別』與『社交加值』以晉升熱區指標。"
            elif final_sfs >= T_EFF: 
                level, color = "高效普及 (Efficiency Access)", "blue"
                advice = "【高效普及】建議：強化供應鏈與外送效率。轉型路徑：建議增加『品牌力介入』與『店裝升級』以轉向社區標準。"
            else: 
                level, color, advice = "戰略排除 (Exclusion)", "red", "📍 數據不足以支持戰略投入。"
            ...
            # 消費行為預判
            if spending_power < 6000 and density > 15: behavior = "價格導向型 (Price-Driven)"
            elif density > 18: behavior = "快速消費型 (Fast/Impulse)"
            elif spending_power > 8000 and social_r > 0.22: behavior = "社交美學導向"
            else: behavior = "品質生活導向"
            ...
            m5.metric("消費行為", behavior.split(' ')[0])

            st.markdown(f"#### 🔍 戰略落點與門檻 (指標: {T_HOT} / {T_STD} / {T_EFF})")
            st.divider()

            l_col, r_col = st.columns(2)
            with l_col:
                st.markdown(f"### 🎯 戰略差距分析\n{gap_info}")
                st.markdown(f"### 🛍️ 消費行為與預判\n* **當前模式**：{behavior}\n* **預判動能**：{'標竿級' if spending_power > 7500 else '普及級'}")
                st.success(f"🏆 **綜合建議與解套路徑**：\n\n{advice}")

            with r_col:
                st.markdown("### 👥 客群結構 (依比例排序)")
...
st.caption("Produced by Marketing Designer. v4.0.1 Build 2026.")
