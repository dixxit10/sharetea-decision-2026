...
# --- 2. 戰略體系規則定義 (直接展示，不使用 Emoji) ---
st.title("Sharetea Express Strategy Suite v8.3")
...
# --- 3. 數據輸入與伺服器端地圖處理 ---
st.sidebar.header("Strategic Data Input")
...
def get_ai_diagnostic(context, api_key):
    ...
    try:
        ...
        prompt = f"""
        身為 Marketing Designer 戰略顧問，針對 200-460 sqft 微型店態進行專業分析。
        背景數據：{context}
        請提供中文專業分析：
        1.【評分地理成因】：分析地圖地理特徵（路幅、鄰里質感）如何導致目前的 SFS 評分。
        2.【轉型執行建議】：若要針對當前空間質量({quality_label})進行轉型，應如何具體執行品牌力介入？
        """
    ...
    if not coord_input:
        ...
        try:
            ...
            with m2:
                ...
                st.metric("Tier Gap", f"{gap_pct:.1%}")
